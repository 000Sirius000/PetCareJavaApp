...
public class ActivityDistanceBarChartView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText("km", 10f, top + 20f, textPaint);
        ...
        for (int i = 0; i < bars.size(); i++) {
            ...
            float height = (float) ((bar.distanceKm / max) * (bottom - top - 10f));
            ...
            canvas.drawText(FormatUtils.number(bar.distanceKm), x + 2f, y - 8f, textPaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        for (BarInfo bar : bars) {
            if (x >= bar.left && x <= bar.right && y >= bar.top && y <= bar.bottom) {
                Toast.makeText(
                        getContext(),
                        bar.label + ": " + FormatUtils.number(bar.distanceKm) + " km · " + bar.sessionCount + " sessions",
                        Toast.LENGTH_SHORT
                ).show();
                return true;
            }
    ...
    private void buildBars() {
        ...
        if (range == FilterRange.YEAR) {
            ...
            for (int month = 0; month < 12; month++) {
                ...
                BarInfo info = sumDistance(start.getTimeInMillis(), end.getTimeInMillis());
    ...
    private BarInfo sumDistance(long start, long end) {
        ...
        for (ActivitySession item : sessions) {
            if (!supportsDistance(item.activityType)) continue;
